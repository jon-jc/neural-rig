...
IPLUG2_ROOT = "..\..\..\reference\NeuralAmpModelerPlugin\iPlug2"
...
def main():
    ...
    if not zip:
        installer = "\\build-win\\installer\\NeuralRig Installer.exe"

        if demo:
            installer = "\\build-win\\installer\\NeuralRig Demo Installer.exe"

        files = [
            projectpath + installer,
            projectpath + "\\installer\\changelog.txt",
            projectpath + "\\installer\\known-issues.txt",
            projectpath + "\\manual\\NeuralRig manual.pdf",
        ]
    else:
        files = [
            projectpath
            + "\\build-win\\NeuralRig.vst3\\Contents\\x86_64-win\\NeuralRig.vst3",
            projectpath + "\\build-win\\NeuralRig_x64.exe",
        ]
    ...
    files = [
        projectpath + "\\build-win\\pdbs\\NeuralRig-vst3_x64.pdb",
        projectpath + "\\build-win\\pdbs\\NeuralRig-app_x64.pdb",
    ]
